...
@Slf4j
@Component
public class MysqlWriteHandler extends AbstractHandler {
    @Override
    public boolean handleBusiness(MsgWrapper msgWrapper, Table table, Rule rule) {
        log.info("mysql处理器：msgWrapper:{},table:{},rule:{}", JSON.toJSONString(msgWrapper), JSON.toJSONString(table), JSON.toJSONString(rule));
        boolean flag = true;
        for (DstCfg dstCfg : rule.getDstCfg()) {
            if (dstCfg.getDsType().equalsIgnoreCase(DsType.MYSQL.getType())) {
                try {
                    OperatorHolder.getOperator(dstCfg.getDsType()).operate(msgWrapper, dstCfg);
                } catch (Exception e) {
                    log.error("mysql处理器执行异常：msgWrapper:{},table:{},rule:{}，e:{}", JSON.toJSONString(msgWrapper), JSON.toJSONString(table), JSON.toJSONString(rule), e);
                    flag = false;
                }
            }
        }
